...
**Файл: `src/domain/resource/value-objects/Namespace.ts`** (обновленный)  `#structure:
...
**Файл: `src/domain/resource/value-objects/ResourceName.ts`** (обновленный)  `#structure:
...
```typescript
import { UuidInvariant, InvariantViolationError } from '@/domain/shared'  #structure:
